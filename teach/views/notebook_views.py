...
def content_nb_view(request,
                    collection_id):
    coll = None
    for obj in [Course, Topic, Lecture,
                Slide, ContentType]:
        try:
    ...
    if isinstance(coll, Course):
        content_filter = {'slide__in': Slide.objects.filter(lecture__in=coll.lectures.all())}
