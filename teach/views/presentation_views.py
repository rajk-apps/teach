...
def course_view(request, course_id):
    ...
    to_include = [content_type.name for content_type
...
def contentshow(request, course_id, type_id):
    course_inst = get_object_or_404(Course,
                                    id=course_id)
    course_structure = course_inst.coursestructure_set.order_by('ordernum')
    # a little crazy list comprehension
    # collecting list of content for each lecture in
    # {'o':ordernum,'c':content} format
    scl = [{'o': content_dic['o'], 'c':c} for content_dic in
           [{'o': cs.ordernum,
             'cl': [ss.content for ls in
                    cs.lecture.lecturestructure_set.order_by('ordernum',
                                                             'subordernum')
                    for ss in
                    ls.slide.slidestructure_set.order_by('ordernum')]}
            for cs in course_structure]
           for c in content_dic['cl']]

    # additional content in list of dicts
    acl = [{'o': ac.afterlecture,
            'c': ac.content} for ac in
           course_inst.additionalcontent_set.order_by('afterlecture')]

    level1 = {'name': type_id,
              'content': []}
        
    ordered_to_add = sorted(scl + acl,
                            key=lambda x: x['o'])
