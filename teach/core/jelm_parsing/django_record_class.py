...
class DjangoRecord:
    ...
    def __init__(self, model_name: str, django_id, jelm_id, attributes):
    ...
    def from_django_dump_record(cls, record_dict: dict):
        ...
        jelm_id = "{}-{}".format(model_name, django_id)
    ...
    @classmethod
    def from_jelm_object(cls, jelm_object: Union[Edge, Node]):
    ...
    def save_to_django_db(self):
        ...
        inst = self.teach_jelm_cls.cls(pk=self.django_id, **self.attributes)
        # inst.save()
    ...
    def _get_jelm_edge(self):
        ...
        if self.teach_jelm_cls.is_edge:
            ...
            source_jelm_id = "{}-{}".format(
                self.teach_jelm_cls.source_model, source_django_id
            )

            target_django_id = record_atts.pop(self.teach_jelm_cls.target_attid)
            target_jelm_id = "{}-{}".format(
                self.teach_jelm_cls.target_model, target_django_id
            )
        ...
        if not self.teach_jelm_cls.is_edge:
            ...
            for add_edge_dic in self.teach_jelm_cls.additional_edges:
                ...
                if isinstance(id_or_list, str):
                    ...
                    raise ValueError(
                        "should be list/str, is {}".format(type(id_or_list))
                    )

                for target_django_id in id_list:
                    target_jelm_id = "{}-{}".format(target_model_name, target_django_id)
