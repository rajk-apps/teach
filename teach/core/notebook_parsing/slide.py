...
class SlideNotebook(NotebookConverter):
    ...
    def parse_object_to_cells(cls, o: Slide) -> list:
        ...
        meta_code = "\n".join(
            ['title = "{}"'.format(o.title), "hastitle = {}".format(o.hastitle)]
        )
        ...
        structure_code = "\n".join(
            [
                parse_ss_to_code(ss)
                for idx, ss in enumerate(
                    o.slidestructure_set.all().order_by("ordernum")
                )
            ]
        )
        ...
        preview_md = "\n".join(
            [
                "**preview on [this](http://146.110.60.131:6969/teach/slide/{}) link on full screen**".format(
                    o.pk
                ),
        ...
        preview_code = "IFrame('http://146.110.60.131:6969/teach/slide/{}', **default_res)".format(
            o.pk
        )

        return [
            cls.code_cell_from_src(meta_code),
