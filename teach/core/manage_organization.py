...
def get_content_occurrences(content):
    out = {}
    lss = [ls for s in content.slide_set.all()
...
def get_struct_dic(model, filter_kwargs,
                   parent_model, subordernum=False):
    if subordernum:
        def sorter_fun(x):
            return (x.__getattribute__(parent_model).pk,
                    x.ordernum, x.subordernum)
    else:
        def sorter_fun(x):
            return (x.__getattribute__(parent_model).pk,
                    x.ordernum)
...
def get_map_md(content_set):
    slide_structure_dic = get_struct_dic(SlideStructure,
                                         {'content__in': content_set},
                                         'slide')

    lecture_structure_dic = get_struct_dic(LectureStructure,
                                           {'slide__in': slide_structure_dic.keys()},
                                           'lecture', subordernum=True)

    course_structure_dic = get_struct_dic(CourseStructure,
                                          {'lecture__in': lecture_structure_dic.keys()},
                                          'course')
